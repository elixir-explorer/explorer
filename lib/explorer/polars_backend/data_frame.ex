defmodule Explorer.PolarsBackend.DataFrame do
  ...
  @impl true
  def to_map(df, convert_series?) do
    for series <- Shared.apply_native(df, :df_get_columns, [names(df)]), reduce: %{} do
      acc ->
        series_name =
          series
          |> Native.s_name()
        ...
        series =
          case convert_series? do
            true -> series |> Shared.to_series() |> PolarsSeries.to_list()
            false -> Shared.to_series(series)
          end

        Map.put_new(acc, series_name, series)
    end
  end
  ...
  def select(df, columns, :drop) when is_list(columns),
    do: Shared.apply_native(df, :df_drop, [columns])
