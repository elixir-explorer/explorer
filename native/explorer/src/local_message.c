...
extern void *enif_open_resource_type(ErlNifEnv *env, const char *module_str, const char *name_str, void (*dtor)(ErlNifEnv *, void *), ErlNifResourceFlags flags, ErlNifResourceFlags *result);
extern ERL_NIF_TERM enif_make_atom(ErlNifEnv *env, const char *name);
extern int enif_get_local_pid(ErlNifEnv *env, ERL_NIF_TERM term, ErlNifPid *pid);
...
int enif_send(ErlNifEnv* caller_env,ErlNifPid* to_pid, ErlNifEnv* msg_env, ERL_NIF_TERM msg);
...
ERL_NIF_TERM local_message_on_gc(ErlNifEnv *env, const ERL_NIF_TERM pid_term, const ERL_NIF_TERM term) {
...
void local_message_open_resource(ErlNifEnv *env) {
    void *rt = enif_open_resource_type(env, "Elixir.Explorer.Remote.LocalGC", "message", destruct_local_message, ERL_NIF_RT_CREATE, 0);
    if (!rt) return;
    message_resource = rt;
