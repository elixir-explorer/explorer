defmodule Explorer.DataFrameTest do
  ...
  describe "filter_with/2" do
    test "filter columns with equal comparison" do
      ...
      df1 = DF.filter_with(df, Series.equal(a, 2))
      assert DF.to_columns(df1, atom_keys: true) == %{a: [2, 2], b: [2.4, 2.0]}

      df2 = DF.filter_with(df, Series.equal(1.0, b))
      assert DF.to_columns(df2, atom_keys: true) == %{a: [3], b: [1.0]}

      df3 = DF.filter_with(df, Series.equal(a, Series.from_list([1, 0, 3, 0])))
      assert DF.to_columns(df3, atom_keys: true) == %{a: [1, 3], b: [5.3, 1.0]}

      df4 = DF.filter_with(df, Series.equal(Series.from_list([0, 0, 1.0, 0]), b))
      assert DF.to_columns(df4, atom_keys: true) == %{a: [3], b: [1.0]}

      df5 = DF.filter_with(df, Series.equal(a, b))
    ...
    test "raise an error if the last operation is an arithmetic operation" do
      ...
      message = "Polars Error: filter predicate was not of type boolean"

      assert_raise RuntimeError, message, fn ->
        DF.filter_with(df, Series.pow(Series.col("a"), 3))
      end
    end
    ...
    test "raise an error if the last operation is an aggregation operation" do
      ...
      message = "Polars Error: filter predicate must be of type `Boolean`, got `i64`"

      assert_raise RuntimeError, message, fn ->
        DF.filter_with(df, Series.sum(Series.col("a")))
      end
    end

    test "raise an error if the input is not a lazy series" do
      df = DF.new(a: [1, 2, 3, 4, 5, 6, 5], b: [9, 8, 7, 6, 5, 4, 3])

      assert_raise FunctionClauseError, fn ->
        DF.filter_with(df, :foo)
      end
    end
  end
  ...
  describe "filter/2" do
    ...
    test "filter with a list of predicates, but one is not boolean" do
      ...
      error_message = "Polars Error: filter predicate was not of type boolean"
