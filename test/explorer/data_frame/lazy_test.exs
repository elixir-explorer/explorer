defmodule Explorer.DataFrame.LazyTest do
  ...
  describe "filter_with/2" do
    test "filters by a simple selector" do
      ...
      ldf1 = DF.filter_with(ldf, Series.greater(Series.col("a"), 2))
      df1 = DF.collect(ldf1)
    ...
    test "filters with a group and without aggregation" do
      ...
      ldf1 = DF.filter_with(ldf_grouped, Series.greater(Series.col("col2"), 2))
    ...
    test "filters with a group and with aggregation" do
      ...
      ldf_grouped1 =
        DF.filter_with(
          ldf_grouped,
          Series.greater(Series.col("col2"), Series.mean(Series.col("col2")))
        )
  ...
  describe "sort_with/2" do
    test "with a simple df and asc order" do
      ...
      ldf1 = DF.sort_with(ldf, asc: Series.col("a"))
    ...
    test "with a simple df one column and without order" do
      ldf = DF.new([a: [1, 2, 4, 3, 6, 5], b: ["a", "b", "d", "c", "f", "e"]], lazy: true)
      ldf1 = DF.sort_with(ldf, Series.col("a"))
    ...
    test "with a simple df and desc order" do
      ldf = DF.new([a: [1, 2, 4, 3, 6, 5], b: ["a", "b", "d", "c", "f", "e"]], lazy: true)
      ldf1 = DF.sort_with(ldf, desc: Series.col("a"))
    ...
    test "with a simple df and sort_by by two columns" do
      ...
      ldf1 = DF.sort_with(ldf, asc: Series.col("a"), asc: Series.col("b"))
    ...
    test "with a simple df and window function" do
      ldf = DF.new([a: [1, 2, 4, 3, 6, 5], b: ["a", "b", "d", "c", "f", "e"]], lazy: true)
      ldf1 = DF.sort_with(ldf, desc: Series.window_mean(Series.col("a"), 2))
    ...
    test "sort considering groups" do
      ...
      sorted_ldf = DF.sort_with(grouped_ldf, desc: Series.col("speed"))
  ...
  describe "mutate_with/2" do
    test "adds new columns" do
      ...
      ldf1 =
        DF.mutate_with(ldf, c: Series.add(Series.col("a"), 5), d: Series.add(2, Series.col("a")))
    ...
    test "adds literal columns" do
      ...
      ldf1 = DF.mutate_with(ldf, a: 1, b: 2.0, c: true)
    ...
    test "changes a column" do
      ...
      ldf1 =
        DF.mutate_with(ldf,
          a: Series.cast(Series.col("a"), {:f, 64})
        )
    ...
    test "calculates aggregations over groups" do
      ...
      ldf2 = DF.mutate_with(ldf1, c: Series.mean(Series.col("a")))
  ...
  describe "summarise_with/2" do
    test "with one group and one column with aggregations", %{ldf: ldf} do
      total = Series.col("total")

      ldf1 =
        ldf
        |> DF.group_by("year")
        |> DF.summarise_with(total_min: Series.min(total), total_max: Series.max(total))
    ...
    test "with one group and two columns with aggregations", %{ldf: ldf} do
      ldf1 =
        ...
        |> DF.group_by("year")
        |> DF.summarise_with(
          total_min: Series.min(Series.col("total")),
          total_max: Series.max(Series.col("total")),
          median_liquid_fuel: Series.median(Series.col("liquid_fuel"))
        )
