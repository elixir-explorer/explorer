defmodule Explorer.DataFrame.GroupedTest do
  ...
  describe "summarise_with/2" do
    ...
    test "with one group and aggregations with addition and subtraction", %{df: df} do
      df1 =
        ...
        |> DF.group_by("year")
        |> DF.summarise_with(
          total_min: Series.min(Series.add(Series.col("total"), 4)),
          total_max: Series.max(Series.subtract(Series.col("total"), Series.col("liquid_fuel")))
        )
    ...
    test "with one group and some aggregations", %{df: df} do
      df1 =
        ...
        |> DF.group_by(["year"])
        |> DF.summarise_with(
          count: Series.count(Series.col("country")),
          standard_deviation: Series.standard_deviation(Series.col("gas_fuel")),
          variance: Series.variance(Series.col("total"))
        )
    ...
    test "with one group, one aggregation with a window function inside", %{df: df} do
      df1 =
        ...
        |> DF.summarise_with(
          count: Series.count(Series.col("country")),
          max_of_win_solid_fuel_mean: Series.max(Series.window_mean(Series.col("solid_fuel"), 2))
        )
    ...
    test "with one group and one window function with one aggregation inside", %{df: df} do
      ...
      assert_raise RuntimeError, message, fn ->
        ...
        |> DF.summarise_with(
          count: Series.count(Series.col("country")),
          max_of_win_solid_fuel_mean: Series.window_mean(Series.max(Series.col("solid_fuel")), 2)
        )
      end
    end
  end
  ...
  describe "sort_by/2" do
    test "sorts by group", %{df: df} do
      ...
      assert grouped_df
             |> DF.ungroup()
             |> DF.filter_with(Series.equal(Series.col("country"), "HONDURAS"))
             |> DF.pull("total")
             |> Series.first() == 2175
    end
  ...
  describe "sort_with/2" do
    test "sorts by group", %{df: df} do
      grouped_df =
        df
        |> DF.group_by("country")
        |> DF.sort_with(asc: Series.col("total"))
  ...
  describe "mutate_with/2" do
    test "adds new columns when there is a group" do
      ...
      df2 =
        DF.mutate_with(df1,
          d: Series.add(Series.col("a"), -7.1),
          e: Series.count(Series.col("c"))
        )
