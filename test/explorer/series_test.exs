defmodule Explorer.SeriesTest do
  ...
  describe "from_list/1" do
    ...
    test "with naive datetimes" do
      ...
      assert Series.from_list(dates, dtype: {:datetime, :micro_seconds}) |> Series.to_list() ==
               dates
  ...
  describe "cast/2" do
    ...
    test "integer series to datetime" do
      s = Series.from_list([1, 2, 3])
      s1 = Series.cast(s, {:datetime, :micro_seconds})
      ...
      assert Series.dtype(s1) == {:datetime, :micro_seconds}

      s2 = Series.from_list([1_649_883_642 * 1_000 * 1_000])
      s3 = Series.cast(s2, {:datetime, :micro_seconds})

      assert Series.to_list(s3) == [~N[2022-04-13 21:00:42.000000]]
      assert Series.dtype(s3) == {:datetime, :micro_seconds}
    end
    ...
    test "error when casting with unknown dtype" do
      error_message =
        "Explorer.Series.cast/2 not implemented for dtype :money. " <>
          "Valid dtypes are [:binary, :boolean, :category, :date, :time, {:datetime, :nano_seconds}, {:datetime, :micro_seconds}, {:datetime, :milli_seconds}, :float, :integer, :string]"
